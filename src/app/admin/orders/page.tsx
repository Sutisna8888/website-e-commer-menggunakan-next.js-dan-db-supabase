  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="flex items-center gap-2">
            <label className="text-sm font-semibold text-brand-gray-600 dark:text-brand-gray-400 hidden sm:block">Waktu:</label>
            <select 
              value={dateRange}
              onChange={(e) => {
              ...
              className="px-4 py-2.5 rounded-xl border border-brand-gray-200 dark:border-brand-dark-700 focus:outline-none focus:ring-2 focus:ring-brand-orange-500 bg-white dark:bg-brand-dark-900 font-semibold text-brand-dark-900 dark:text-white"
            >
          ...
          {dateRange === 'CUSTOM' && (
            <div className="flex items-center gap-2 bg-brand-gray-50 dark:bg-brand-dark-800 p-1.5 rounded-xl border border-brand-gray-200 dark:border-brand-dark-700">
              <input 
                type="date" 
                ...
                className="px-2 py-1 bg-transparent text-sm focus:outline-none dark:text-white dark:color-scheme-dark"
              />
              <span className="text-brand-gray-400 dark:text-brand-gray-500 font-bold">-</span>
              <input 
                type="date" 
                value={customEndDate}
                onChange={e => {
          ...
          <div className="flex items-center gap-2 border-l border-brand-gray-200 dark:border-brand-dark-700 pl-4">
            <label className="text-sm font-semibold text-brand-gray-600 dark:text-brand-gray-400 hidden lg:block">Status:</label>
            <select 
              value={statusFilter}
              onChange={(e) => {
      ...
      <div className="bg-white dark:bg-brand-dark-900/50 backdrop-blur-xl rounded-3xl shadow-sm border border-brand-gray-100 dark:border-brand-dark-800 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-12 text-center text-brand-gray-500 dark:text-brand-gray-400">
                    Tidak ada pesanan yang ditemukan
                  </td>
                </tr>
              ) : (
                orders.map((order) => (
                  <tr key={order.id} className="border-b border-brand-gray-50 dark:border-brand-dark-800/50 hover:bg-brand-gray-50/50 dark:hover:bg-brand-dark-800/30 transition-colors">
                    <td className="py-4 px-6 align-top">
                      <p className="font-bold text-brand-dark-900 dark:text-white uppercase text-xs">#{order.id.slice(0, 8)}</p>
                      <p className="text-xs text-brand-gray-500 dark:text-brand-gray-400 mt-1">{formatDate(order.createdAt)}</p>
                    </td>
                    <td className="py-4 px-6 align-top">
                      <p className="font-bold text-brand-dark-900 dark:text-white">{order.user.name || 'User'}</p>
                      <p className="text-xs text-brand-gray-500 dark:text-brand-gray-400">{order.user.email}</p>
                      <p className="text-[10px] text-brand-gray-400 dark:text-brand-gray-500 mt-2 truncate max-w-[200px]">{order.address}</p>
                    </td>
                    <td className="py-4 px-6 align-top">
                      <ul className="space-y-1">
                        {order.orderItems.map((item, i) => (
                          <li key={i} className="text-xs">
                            <span className="font-semibold text-brand-dark-900 dark:text-white">{item.quantity}x {item.foodItem.name}</span>
                            <div className="text-[10px] text-brand-gray-500 dark:text-brand-gray-400 ml-4">
                    ...
                    <td className="py-4 px-6 align-top font-bold text-brand-orange-600 dark:text-brand-orange-500">
                      {formatPrice(order.totalAmount)}
                      <p className="text-[10px] text-brand-gray-500 dark:text-brand-gray-400 font-normal mt-1">{order.paymentMethod}</p>
                    </td>
                    <td className="py-4 px-6 align-top">
                    ...
                    <td className="py-4 px-6 align-top text-right">
                      {updatingId === order.id ? (
                        ...
                        <select 
                          ...
                          className="px-2 py-1.5 rounded-lg border border-brand-gray-200 dark:border-brand-dark-700 text-xs font-semibold text-brand-dark-900 dark:text-white bg-white dark:bg-brand-dark-900 focus:outline-none focus:border-brand-orange-500"
                        >
                          {statusOptions.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      )}
        ...
        {/* Pagination UI */}
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-brand-gray-100 dark:border-brand-dark-700 bg-brand-gray-50 dark:bg-brand-dark-800/50">
            <span className="text-sm text-brand-gray-500 dark:text-brand-gray-400">
              Halaman <span className="font-bold text-brand-dark-900 dark:text-white">{page}</span> dari <span className="font-bold text-brand-dark-900 dark:text-white">{totalPages}</span>
            </span>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page === 1}
                className="px-3 py-1.5 text-sm font-semibold rounded-lg bg-white dark:bg-brand-dark-900 border border-brand-gray-200 dark:border-brand-dark-700 text-brand-dark-900 dark:text-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-brand-gray-100 dark:hover:bg-brand-dark-800 transition-colors"
              >
                Sebelumnya
              </button>
              <button
